defmodule HydraX.Report do
  ...
  defp render_role_queue_dispatch_pass(pass) do
    ...
    delegation_expanded = render_scheduler_count(pass, "delegation_expanded_count")
    delegation_deferred = render_scheduler_count(pass, "delegation_deferred_count")
    ...
    "processed=#{processed}; delegation_expanded=#{delegation_expanded}; delegation_deferred=#{delegation_deferred}; required_role_prioritized=#{required_role_prioritized}; pressure_skipped=#{pressure_skipped}; remote_owned=#{remote_owned}; skipped=#{skipped}; errors=#{errors}; owner=#{owner}"
  end
  ...
  defp report_delegation_deferral_batches(entry) when is_map(entry) do
    ...
    if repeated > 0 or max_deferrals > 1 do
      " repeat_deferrals=#{repeated} max_deferrals=#{max_deferrals}"
    else
      ""
    end
  end
  ...
  defp report_recovery_mix_or_none(mix) when is_map(mix) and map_size(mix) > 0,
  ...
  defp delegation_batch_budget_detail(%{
         "batch_budget" => budget,
         "supervision_active_batches" => active_batches
       })
       when is_integer(budget) and is_integer(active_batches) do
    "delegation_batch_budget=#{budget} delegation_active_batches=#{active_batches}"
  end

  defp delegation_batch_budget_detail(_snapshot), do: nil
