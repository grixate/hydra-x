defmodule HydraXWeb.ArchitectureNodeAPIController do
  ...
  def create(conn, %{"project_id" => project_id, "architecture_node" => params}) do
    with {:ok, %GraphNode{} = node} <- Product.create_architecture_node(project_id, params) do
  ...
  def update(conn, %{"project_id" => project_id, "id" => id, "architecture_node" => params}) do
    ...
    with {:ok, %GraphNode{} = updated} <- Product.update_architecture_node(node, params) do
  ...
  def delete(conn, %{"project_id" => project_id, "id" => id}) do
    ...
    with {:ok, %GraphNode{}} <- Product.delete_architecture_node(node) do
