defmodule HydraXWeb.ConstraintAPIController do
  ...
  def create(conn, %{"project_id" => project_id, "constraint" => params}) do
    with {:ok, %GraphNode{} = constraint} <- Product.create_constraint(project_id, params) do
  ...
  def update(conn, %{"project_id" => project_id, "id" => id, "constraint" => params}) do
    ...
    with {:ok, %GraphNode{} = updated} <- Product.update_constraint(constraint, params) do
  ...
  def delete(conn, %{"project_id" => project_id, "id" => id}) do
    ...
    with {:ok, %GraphNode{}} <- Product.delete_constraint(constraint) do
