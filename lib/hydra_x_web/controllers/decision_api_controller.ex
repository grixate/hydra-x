defmodule HydraXWeb.DecisionAPIController do
  ...
  def create(conn, %{"project_id" => project_id, "decision" => params}) do
    with {:ok, %GraphNode{} = decision} <- Product.create_decision(project_id, params) do
  ...
  def update(conn, %{"project_id" => project_id, "id" => id, "decision" => params}) do
    ...
    with {:ok, %GraphNode{} = updated} <- Product.update_decision(decision, params) do
  ...
  def delete(conn, %{"project_id" => project_id, "id" => id}) do
    ...
    with {:ok, %GraphNode{}} <- Product.delete_decision(decision) do
