defmodule HydraXWeb.InsightAPIController do
  ...
  def create(conn, %{"project_id" => project_id, "insight" => params}) do
    with {:ok, %GraphNode{} = insight} <- Product.create_insight(project_id, params) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"project_id" => project_id, "id" => id, "insight" => params}) do
    ...
    with {:ok, %GraphNode{} = updated} <- Product.update_insight(insight, params) do
  ...
  def delete(conn, %{"project_id" => project_id, "id" => id}) do
    ...
    with {:ok, %GraphNode{}} <- Product.delete_insight(insight) do
