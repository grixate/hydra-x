defmodule HydraXWeb.KnowledgeEntryAPIController do
  ...
  def create(conn, %{"project_id" => project_id, "knowledge_entry" => params}) do
    with {:ok, %GraphNode{} = entry} <- Product.create_knowledge_entry(project_id, params) do
  ...
  def update(conn, %{"project_id" => project_id, "id" => id, "knowledge_entry" => params}) do
    ...
    with {:ok, %GraphNode{} = updated} <- Product.update_knowledge_entry(entry, params) do
  ...
  def delete(conn, %{"project_id" => project_id, "id" => id}) do
    ...
    with {:ok, %GraphNode{}} <- Product.delete_knowledge_entry(entry) do
  ...
  def review(conn, %{"project_id" => project_id, "id" => id} = params) do
    ...
    with {:ok, %GraphNode{} = updated} <- Product.review_knowledge(entry, action, edits) do
      json(conn, %{data: ProductPayload.knowledge_entry_json(updated)})
    end
  end
end
