defmodule HydraX.Graph.IntegrationTest do
  ...
  current product-development workflows. Walks through:
    ...
    1. Create a project and apply the product_development pretrained
       project to give it a schema.
  ...
  test "insight + source + supports + flag lifecycle", %{project: project} do
    ...
    {:ok, edge} = Relationships.create_relationship(source, insight, "supports")
    assert edge.extends_primitive == "supports"

    claim_supports = Relationships.list_by_primitive(project.id, "supports")
  ...
  test "raising an unknown flag type is rejected", %{project: project} do
    {:ok, node} =
      Nodes.create_node(project.id, %{
        type_key: "insight",
        title: "x",
  ...
  test "cross-project relationships are rejected", %{project: project} do
    ...
    :ok = ProductDevelopment.apply_to_project(other_project.id)
    ...
    {:ok, b} =
      Nodes.create_node(other_project.id, %{
        type_key: "insight",
        title: "B",
        status: "draft"
      })
