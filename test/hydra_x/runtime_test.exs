defmodule HydraX.RuntimeTest do
  ...
  test "guided constraint replans inherit narrowed fallback fan-out" do
    planner =
      create_agent()
      |> then(&Runtime.get_agent!(&1.id))
    ...
    {:ok, parent} =
      Runtime.save_work_item(%{
        "kind" => "research",
        "goal" => "Recover a constrained delegated branch with a guided fallback.",
        "assigned_agent_id" => planner.id,
        "assigned_role" => "planner",
        "execution_mode" => "delegate",
        "priority" => 7,
  ...
  test "guided constraint replans de-escalate under existing operator-guided pressure" do
    ...
    assert get_in(replan_item.metadata || %{}, ["recovery_strategy_pressure_snapshot"]) == %{
             ...
             "planner_active_intervention_portfolios" => 1,
  ...
  test "guided constraint replans avoid fallback-contended alternatives when de-escalating" do
    planner =
      create_agent()
      |> then(&Runtime.get_agent!(&1.id))
    ...
    {:ok, _fallback_pressure_parent} =
      Runtime.save_work_item(%{
        "kind" => "research",
        "goal" => "Keep a constraint-first delegation tree with reviewer fallback open.",
        "assigned_agent_id" => planner.id,
        "assigned_role" => "planner",
        "status" => "blocked",
        "execution_mode" => "delegate",
          ...
          "follow_up_summary" => %{
            ...
            "entries" => [
              %{
                ...
                "alternative_strategies" => ["review_guided_replan"],
                "alternative_summaries" => ["Reviewer-guided recovery"],
                "priority_boost" => 1
              }
            ]
          }
    ...
    {:ok, parent} =
      Runtime.save_work_item(%{
        "kind" => "research",
        "goal" =>
          "Recover a constrained branch without piling onto fallback-contended review guidance.",
        "assigned_agent_id" => planner.id,
        "assigned_role" => "planner",
        "execution_mode" => "delegate",
        "priority" => 7,
    ...
    assert get_in(replan_item.metadata || %{}, ["recovery_strategy_pressure_snapshot"]) == %{
             ...
             "base_selected_count" => 1,
             "planner_active_intervention_portfolios" => 1,
             "preferred" => "request_review",
             "preferred_selected_count" => 0,
             "preferred_fallback_count" => 0,
             "alternative_selected_counts" => %{"operator_guided_replan" => 1},
             "alternative_fallback_counts" => %{"review_guided_replan" => 1}
           }
  ...
  test "guided constraint replans prefer lighter de-escalation under sustained operator pressure" do
    planner =
      create_agent()
      |> then(&Runtime.get_agent!(&1.id))
    ...
    for index <- 1..2 do
      {:ok, _pressure_parent} =
        Runtime.save_work_item(%{
          "kind" => "research",
          "goal" => "Keep operator-guided delegation tree #{index} open.",
          "assigned_agent_id" => planner.id,
          "assigned_role" => "planner",
          "status" => "blocked",
          "execution_mode" => "delegate",
    ...
    {:ok, parent} =
      Runtime.save_work_item(%{
        "kind" => "research",
        "goal" => "Recover a constrained branch while avoiding more operator-guided load.",
        "assigned_agent_id" => planner.id,
        "assigned_role" => "planner",
        "execution_mode" => "delegate",
        "priority" => 7,
    ...
    assert get_in(replan_item.metadata || %{}, ["recovery_strategy_selection_reason"]) ==
             "de-escalated from Operator-guided recovery under sustained planner recovery pressure (2 existing)"
    ...
    assert get_in(replan_item.metadata || %{}, ["recovery_strategy_pressure_snapshot"]) == %{
             ...
             "preferred_selected_count" => 0,
             "preferred_fallback_count" => 0,
             "alternative_selected_counts" => %{"operator_guided_replan" => 2},
             "alternative_fallback_counts" => %{}
           }
  ...
  test "guided constraint replans avoid candidates already under de-escalated pressure" do
    ...
    {:ok, _deescalated_review_parent} =
      Runtime.save_work_item(%{
        "kind" => "research",
        "goal" => "Keep a review-guided path already stepped down to constraint-first.",
        "assigned_agent_id" => planner.id,
        "assigned_role" => "planner",
        "status" => "blocked",
        "execution_mode" => "delegate",
          ...
          "follow_up_summary" => %{
            ...
            "entries" => [
              %{
                ...
                "strategy" => "constraint_replan",
                "summary" => "Constraint-first recovery",
                "deescalated_from" => "review_guided_replan",
                "priority_boost" => 1
              }
            ]
          }
    ...
    {:ok, parent} =
      Runtime.save_work_item(%{
        "kind" => "research",
        "goal" =>
          "Recover a constrained branch without piling onto a review path already being stepped down.",
        "assigned_agent_id" => planner.id,
        "assigned_role" => "planner",
        "execution_mode" => "delegate",
        "priority" => 7,
    ...
    assert get_in(replan_item.metadata || %{}, ["recovery_strategy_pressure_snapshot"]) == %{
             ...
             "alternative_selected_counts" => %{"operator_guided_replan" => 1},
             "alternative_fallback_counts" => %{},
             "alternative_deescalated_counts" => %{"review_guided_replan" => 1}
           }
  ...
  test "guided constraint replans de-escalate under existing de-escalated operator pressure" do
    ...
    assert get_in(replan_item.metadata || %{}, ["recovery_strategy_pressure_snapshot"]) == %{
             "alternative_deescalated_counts" => %{"operator_guided_replan" => 1},
             "base" => "operator_guided_replan",
             "base_selected_count" => 0,
  ...
  test "review-guided constraint replans prefer constraint-first recovery under sustained review pressure" do
    ...
    for index <- 1..2 do
      {:ok, _pressure_parent} =
        Runtime.save_work_item(%{
          "kind" => "research",
          "goal" => "Keep reviewer-guided delegation tree #{index} open.",
          "assigned_agent_id" => planner.id,
          "assigned_role" => "planner",
          "status" => "blocked",
          "execution_mode" => "delegate",
    ...
    {:ok, parent} =
      Runtime.save_work_item(%{
        "kind" => "research",
        "goal" => "Recover a constrained branch while avoiding more reviewer-guided load.",
        "assigned_agent_id" => planner.id,
        "assigned_role" => "planner",
        "execution_mode" => "delegate",
        "priority" => 7,
    ...
    assert get_in(replan_item.metadata || %{}, ["recovery_strategy_selection_reason"]) ==
             "de-escalated from Reviewer-guided recovery under sustained planner recovery pressure (2 existing)"

    assert get_in(replan_item.metadata || %{}, ["recovery_strategy_pressure_snapshot"]) == %{
             "base" => "review_guided_replan",
             "base_selected_count" => 2,
             "planner_active_intervention_portfolios" => 2,
             "preferred" => "constraint_replan",
             "preferred_selected_count" => 0,
             "preferred_fallback_count" => 0,
             "alternative_selected_counts" => %{"review_guided_replan" => 2},
             "alternative_fallback_counts" => %{}
           }
  ...
  test "operator-guided constraint replans prefer constraint-first recovery under active intervention portfolio pressure" do
    planner =
      create_agent()
      |> then(&Runtime.get_agent!(&1.id))
    ...
    {:ok, _review_guided_parent} =
      Runtime.save_work_item(%{
        "kind" => "research",
        "goal" => "Keep reviewer-guided delegation tree open.",
        "assigned_agent_id" => planner.id,
        "assigned_role" => "planner",
        "status" => "blocked",
        "execution_mode" => "delegate",
    ...
    {:ok, _request_review_parent} =
      Runtime.save_work_item(%{
        "kind" => "research",
        "goal" => "Keep review-requested delegation tree open.",
        "assigned_agent_id" => planner.id,
        "assigned_role" => "planner",
        "status" => "blocked",
        "execution_mode" => "delegate",
    ...
    {:ok, parent} =
      Runtime.save_work_item(%{
        "kind" => "research",
        "goal" =>
          "Recover a constrained branch while the planner already carries guided recovery load.",
        "assigned_agent_id" => planner.id,
        "assigned_role" => "planner",
        "execution_mode" => "delegate",
        "priority" => 7,
    ...
    assert get_in(replan_item.metadata || %{}, ["recovery_strategy_selection_reason"]) ==
             "de-escalated from Operator-guided recovery under active planner intervention portfolio pressure (2 existing)"
  ...
  test "delegation pressure replans narrow the next planner delegation batch" do
    planner =
      create_agent()
      |> then(&Runtime.get_agent!(&1.id))
    ...
    {:ok, parent} =
      Runtime.save_work_item(%{
        "kind" => "research",
        "goal" => "Re-plan a delegation tree under repeated pressure.",
        "assigned_agent_id" => planner.id,
        "assigned_role" => "planner",
        "execution_mode" => "delegate",
        "priority" => 9,
    ...
    assert [
             %{
               "goal" => "Research second constrained branch.",
               "assigned_role" => "researcher"
             }
           ] =
             Enum.map(
               get_in(replan_item.metadata || %{}, ["delegate_batch"]) || [],
               &Map.take(&1, ["goal", "assigned_role"])
             )
    ...
    assert replan_item.status == "blocked"
    assert length(replan_item.result_refs["child_work_item_ids"]) == 1
    assert replan_child.goal == "Research second constrained branch."
  ...
  test "planner prefers the older waiting delegation batch when capacity is otherwise equal" do
    ...
    {:ok, first_parent} =
      Runtime.save_work_item(%{
        "kind" => "research",
        "goal" => "Supervise the first equal-capacity branch.",
        "assigned_agent_id" => planner.id,
        "assigned_role" => "planner",
        "execution_mode" => "delegate",
        "priority" => 8,
        "metadata" => %{
          ...
          "delegate_batch" => [
            ...
            %{"goal" => "Research second branch A.", "role" => "researcher"}
          ]
        }
      })
    ...
    {:ok, second_parent} =
      Runtime.save_work_item(%{
        "kind" => "research",
        "goal" => "Supervise the second equal-capacity branch.",
        "assigned_agent_id" => planner.id,
        "assigned_role" => "planner",
        "execution_mode" => "delegate",
        "priority" => 8,
        "metadata" => %{
          ...
          "delegate_batch" => [
            ...
            %{"goal" => "Research second branch B.", "role" => "researcher"}
          ]
        }
      })
    ...
    first_parent_state = Runtime.get_work_item!(first_parent.id)

    {:ok, first_parent_with_history} =
      Runtime.save_work_item(first_parent_state, %{
        "metadata" => %{
          "delegation_batch" => %{
            "expected_count" => 2,
            "batch_concurrency" => 1,
            ...
            "items" =>
              Runtime.delegation_batch_snapshot(first_parent_state)
              |> Map.get("items", [])
          }
        }
      })
    ...
    refreshed_first = Runtime.delegation_batch_snapshot(first_parent_with_history)
    refreshed_second = Runtime.delegation_batch_snapshot(Runtime.get_work_item!(second_parent.id))

    assert refreshed_first["expansion_count"] == 1
    assert refreshed_second["expansion_count"] == 1
  ...
  test "planner seeds a blocked delegation batch without dispatching children when the shared supervision budget is already exhausted" do
    ...
    {:ok, active_parent} =
      Runtime.save_work_item(%{
        "kind" => "research",
        "goal" => "Supervise the already active branch.",
        "assigned_agent_id" => planner.id,
        "assigned_role" => "planner",
        "execution_mode" => "delegate",
        "priority" => 8,
        "metadata" => %{
          ...
          "delegate_batch_concurrency" => 1,
          "delegate_batch" => [
            %{"goal" => "Research active branch A.", "role" => "researcher"},
            %{"goal" => "Research active branch B.", "role" => "researcher"}
          ]
        }
      })
    ...
    {:ok, queued_parent} =
      Runtime.save_work_item(%{
        "kind" => "research",
        "goal" => "Supervise the queued branch.",
        "assigned_agent_id" => planner.id,
        "assigned_role" => "planner",
        "execution_mode" => "delegate",
        "priority" => 8,
        "metadata" => %{
          ...
          "delegate_batch_concurrency" => 1,
          "delegate_batch" => [
            %{"goal" => "Research queued branch A.", "role" => "researcher"},
            %{"goal" => "Research queued branch B.", "role" => "researcher"}
          ]
        }
      })
    ...
    assert queued_snapshot["expansion_deferred_reason"] == "planner_budget_constrained"
    assert queued_snapshot["supervision_budget"] == 1
  ...
  test "planner expands the delegation batch with healthier pending role capacity first" do
    ...
    {:ok, research_parent} =
      Runtime.save_work_item(%{
        "kind" => "research",
        "goal" => "Supervise the researcher backlog branch.",
        "assigned_agent_id" => planner.id,
        "assigned_role" => "planner",
        "execution_mode" => "delegate",
        "priority" => 8,
        "metadata" => %{
          ...
          "delegate_batch" => [
            ...
            %{"goal" => "Research second branch.", "role" => "researcher"}
          ]
        }
      })
    ...
    {:ok, operator_parent} =
      Runtime.save_work_item(%{
        "kind" => "task",
        "goal" => "Supervise the operator branch.",
        "assigned_agent_id" => planner.id,
        "assigned_role" => "planner",
        "execution_mode" => "delegate",
        "priority" => 8,
        "metadata" => %{
          ...
          "delegate_batch" => [
            ...
            %{"goal" => "Prepare second operator branch.", "role" => "operator", "kind" => "task"}
          ]
        }
      })
    ...
    assert length(expanded_research_parent.result_refs["child_work_item_ids"] || []) == 1
  end

  test "planner prefers the batch missing a required role when expansion capacity is otherwise equal" do
    planner =
      create_agent()
      |> then(&Runtime.get_agent!(&1.id))
    ...
    {:ok, urgent_parent} =
      Runtime.save_work_item(%{
        "kind" => "research",
        "goal" => "Supervise the role-urgent branch.",
        "assigned_agent_id" => planner.id,
        "assigned_role" => "planner",
        "execution_mode" => "delegate",
        "priority" => 8,
        "metadata" => %{
          ...
          "delegate_batch" => [
            %{"goal" => "Research the urgent branch.", "role" => "researcher"},
            %{"goal" => "Research the trailing urgent branch.", "role" => "researcher"},
            %{
              "goal" => "Prepare the operator handoff for the urgent branch.",
              "role" => "operator",
              "kind" => "task"
            }
          ]
    ...
    {:ok, plain_parent} =
      Runtime.save_work_item(%{
        "kind" => "research",
        "goal" => "Supervise the plain branch.",
        "assigned_agent_id" => planner.id,
        "assigned_role" => "planner",
        "execution_mode" => "delegate",
        "priority" => 8,
        "metadata" => %{
          ...
          "delegate_batch_concurrency" => 1,
          "delegate_batch" => [
            %{"goal" => "Research the plain branch.", "role" => "researcher"},
            %{"goal" => "Research the second plain branch.", "role" => "researcher"}
          ]
        }
      })

    assert {:ok, first_delegate} = Runtime.run_autonomy_cycle(planner.id)
    assert first_delegate.action == "delegated"

    assert {:ok, second_delegate} = Runtime.run_autonomy_cycle(planner.id)
    assert second_delegate.action == "delegated"
    ...
    urgent_snapshot = Runtime.delegation_batch_snapshot(Runtime.get_work_item!(urgent_parent.id))
    plain_snapshot = Runtime.delegation_batch_snapshot(Runtime.get_work_item!(plain_parent.id))

    assert urgent_snapshot["missing_completion_roles"] == %{"operator" => 1}
    assert plain_snapshot["missing_completion_roles"] == %{}
    ...
    assert length(expanded_urgent_parent.result_refs["child_work_item_ids"] || []) == 2
  ...
  test "autonomy status reports configured roles and recent work items" do
    ...
    assert Enum.any?(
             ...
             &(&1.agent_id == agent.id and
                 ...
                 (&1.deescalated_recovery_mix || %{}) == %{"operator_guided_replan" => 1} and
                 (&1.deescalated_batches || 0) == 1 and
                 (&1.deescalation_pressure_total || 0) == 1 and
    ...
    assert status.delegation_deescalation_pressure_total == 1
    ...
    assert status.delegation_deescalated_recovery_batches == %{"operator_guided_replan" => 1}
