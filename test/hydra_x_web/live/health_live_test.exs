defmodule HydraXWeb.HealthLiveTest do
  ...
  test "health page shows autonomy posture", %{conn: conn} do
    ...
    assert html =~ "intervention batches 2"
    ...
    assert html =~ "selected-active portfolios"
    assert html =~ "selected-stale portfolios"
    assert html =~ "fallback-heavy portfolios"
    assert html =~ "de-escalated portfolios"
    assert html =~ "selected recovery operator-guided x1"
    assert html =~ "fallback recovery review-guided x1"
    assert html =~ "de-escalated recovery operator-guided x1"
